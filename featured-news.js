...
function addStyles(){
  ...
  style.textContent=`
    .featured-today{padding:28px 0 18px;background:linear-gradient(135deg,#eff6ff 0%,#fff 55%,#f0f9ff 100%);border-bottom:1px solid #dbeafe;overflow:hidden}
    .featured-today-card{display:grid;grid-template-columns:1.05fr .95fr;min-height:330px;border-radius:28px;overflow:hidden;background:linear-gradient(135deg,#075985,#0284c7);box-shadow:0 22px 55px rgba(2,132,199,.22)}
    .featured-today-content{padding:38px 42px;display:flex;flex-direction:column;justify-content:center;color:#fff}
    .featured-today-badge{display:inline-flex;align-items:center;width:max-content;padding:8px 13px;border-radius:999px;background:#fbbf24;color:#172554;font-size:11px;font-weight:900;letter-spacing:.5px}
    ...
    @media(max-width:760px){.featured-today{padding:18px 0 12px}.featured-today-card{grid-template-columns:1fr;min-height:0}.featured-today-content{padding:28px 25px 25px}.featured-today-image-wrap{min-height:230px;order:-1}.featured-today-image{min-height:230px;max-height:300px}}
  `;
