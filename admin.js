...
const modules = {
  ...
  teachers:{label:"👩‍🏫 Guru & Tendik",table:"teachers",fields:[["name","Nama","text"],["nip","NIP","text"],["nuptk","NUPTK","text"],["gender","Jenis Kelamin","select:L,P"],["position","Jabatan","text"],["subject","Mata Pelajaran","text"],["education","Pendidikan","text"],["email","Email","email"],["phone","Telepon","text"],["bio","Biografi","textarea"],["is_active","Aktif","checkbox"],["sort_order","Urutan","number"]]},
...
function openForm(key,row){
  ...
  modalBody.innerHTML=`<form id="form">${m.fields.map(f=>fieldHtml(f,row?.[f[0]])).join("")}<div class="actions"><button class="btn primary">Simpan</button></div></form>`;
  modal.classList.remove("hidden");
  $("form").onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);const payload={};m.fields.forEach(([k,,t])=>{payload[k]=t==="checkbox"?fd.has(k):(fd.get(k)||null)});let r=row?await supabase.from(m.table).update(payload).eq("id",row.id):await supabase.from(m.table).insert(payload);if(r.error)toast(r.error.message);else{toast("Berhasil disimpan");modal.classList.add("hidden");renderModule(key)}};
}
