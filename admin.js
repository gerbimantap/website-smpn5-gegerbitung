...
const modules = {
  ...
  news:{label:"📰 Berita",table:"news",fields:[["title","Judul","text"],["slug","Slug","text"],["excerpt","Ringkasan","textarea"],["content","Isi Berita","textarea"],["category","Kategori","text"],["published_at","Tanggal Publikasi","datetime-local"],["status","Status","select:draft,published,archived"],["is_featured","Unggulan","checkbox"]]},
  ...
  achievements:{label:"🏆 Prestasi",table:"achievements",fields:[["title","Prestasi","text"],["student_name","Nama Siswa","text"],["category","Kategori","text"],["level","Tingkat","text"],["ranking","Peringkat","text"],["year","Tahun","number"],["description","Deskripsi","textarea"]]},
  curriculum_activities:{label:"📚 Kegiatan Kurikulum",table:"curriculum_activities",fields:[["title","Judul","text"],["slug","Slug","text"],["description","Deskripsi","textarea"],["content","Isi","textarea"],["activity_date","Tanggal","date"],["status","Status","select:draft,published,archived"]]},
  student_activities:{label:"👥 Kegiatan Kesiswaan",table:"student_activities",fields:[["title","Judul","text"],["slug","Slug","text"],["description","Deskripsi","textarea"],["content","Isi","textarea"],["activity_date","Tanggal","date"],["status","Status","select:draft,published,archived"]]},
...
function fieldHtml(f,val=""){
  ...
  if(type==="file-image"){
    ...
    return `<div class="field"><label>${label}</label><input type="file" name="${key}_file" accept="image/jpeg,image/png,image/webp">${preview}<input type="hidden" name="${key}" value="${esc(val)}"></div>`;
  }
...
function buildMenu(){
  menu.innerHTML=`<button class="active" data-key="dashboard">📊 Dashboard</button>`+
    Object.entries(modules).map(([k,v])=>`<button data-key="${k}">${v.label}</button>`).join("");
...
async function render(key){
  ...
  if(key==="dashboard"){await renderDashboard();return}
...
async function renderDashboard(){
  ...
  content.innerHTML=`<div class="dashboard-cards">${counts.slice(0,8).map(([k,c])=>`<div class="dash-card"><b>${c}</b><span>${modules[k].label.replace(/^\\S+\\s/,"")}</span></div>`).join("")}</div>
  <div class="panel" style="margin-top:20px"><h2>Selamat datang, Super Admin</h2><p class="muted">Dari dashboard ini Anda dapat mengelola konten website SMPN 5 Gegerbitung. Perubahan akan tersimpan langsung di Supabase.</p></div>`;
}
async function renderModule(key){
  const m=modules[key]; let q=supabase.from(m.table).select("*").order("created_at",{ascending:false});
  const {data,error}=await q;
  if(error){content.innerHTML=`<div class="panel"><div class="error">${esc(error.message)}</div></div>`;return}
  content.innerHTML=`<div class="panel"><div class="actions" style="justify-content:space-between"><p class="muted">Kelola data ${m.label.toLowerCase()}.</p><button class="btn primary" id="add">+ Tambah Data</button></div>
  <div class="table-wrap"><table class="table"><thead><tr>${m.fields.slice(0,5).map(f=>`<th>${f[1]}</th>`).join("")}<th>Aksi</th></tr></thead><tbody>${(data||[]).map(row=>`<tr>${m.fields.slice(0,5).map(f=>`<td>${f[2]==="checkbox"?(row[f[0]]?"Ya":"Tidak"):esc(row[f[0]])}</td>`).join("")}<td><div class="actions"><button class="btn ghost small edit" data-id="${row.id}">Edit</button><button class="btn small" style="background:#fee2e2;color:#b91c1c" data-id="${row.id}" data-del="1">Hapus</button></div></td></tr>`).join("") || `<tr><td colspan="6">Belum ada data.</td></tr>`}</tbody></table></div></div>`;
...
function safeFileName(name){
  return String(name||"foto").toLowerCase().replace(/[^a-z0-9._-]+/g,"-").replace(/-+/g,"-");
}
async function uploadTeacherPhoto(file, teacherId){
  const ext=(file.name.split(".").pop()||"jpg").toLowerCase().replace(/[^a-z0-9]/g,"");
  const path=`teachers/${teacherId}-${Date.now()}.${ext}`;
  const upload=await supabase.storage.from("school-media").upload(path,file,{upsert:true,contentType:file.type||"image/jpeg"});
...
function openForm(key,row){
  ...
  $("form").onsubmit=async e=>{
    ...
    m.fields.forEach(([k,,t])=>{
      if(t==="file-image") payload[k]=fd.get(k)||null;
      else payload[k]=t==="checkbox"?fd.has(k):(fd.get(k)||null);
    });
    try{
      let savedId=row?.id;
      let r;
      if(row){
        r=await supabase.from(m.table).update(payload).eq("id",row.id);
      }else{
        r=await supabase.from(m.table).insert(payload).select("id").single();
        savedId=r.data?.id;
      }
      if(r.error) throw r.error;

      if(key==="teachers"){
        const file=fd.get("photo_path_file");
        if(file && file.size){
          if(!savedId) throw new Error("ID data guru tidak ditemukan setelah disimpan.");
          const path=await uploadTeacherPhoto(file,savedId);
          const u=await supabase.from("teachers").update({photo_path:path}).eq("id",savedId);
          if(u.error) throw u.error;
        }
      }
      toast("Berhasil disimpan"); modal.classList.add("hidden"); renderModule(key);
    }catch(err){
